...
export const LuminaProvider = ({ children }) => {
  ...
  const updateProjectSettings = useCallback(async (sysPrompt) => {
      ...
      if(updated) { setActiveProject(updated); setProjects(p => p.map(x => x.id === updated.id ? updated : x)); }
  }, [activeProject]);
  ...
  const contextValue = useMemo(() => ({
    ...
    settings, updateSettings, // Exposed for Zenith
    sessions, sessionId, startNewChat, loadSession, deleteSession, renameChat, 
    projects, activeProject, setActiveProject, createProject, deleteProject, updateProjectSettings, addFiles, addFolder, addUrl,
    gitStatus, theme, currentView, setCurrentView, 
    calendarEvents, addEvent, updateEvent, deleteEvent, generateSchedule, 
    isInitialized, activeArtifact, setActiveArtifact, 
  ...
  }), [messages, isLoading, isOllamaRunning, currentModel, availableModels, settings, sessions, sessionId, projects, activeProject, gitStatus, theme, currentView, calendarEvents, isInitialized, activeArtifact, isSettingsOpen, canvasNodes, canvasConnections, isSpeaking]);
