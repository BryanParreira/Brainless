...
const QuickActions = ({ onAction, settings, theme }) => {
  const studentActions = [{ label: 'Explain Simple', icon: BookOpen, cmd: '/explain_simple' }, { label: 'Quiz Me', icon: BrainCircuit, cmd: '/quiz' }, { label: 'Study Guide', icon: GraduationCap, cmd: '/study' }, { label: 'Outline Essay', icon: PenTool, cmd: '/outline' }];
  const devActions = [{ label: 'Code Review', icon: Eye, cmd: '/review' }, { label: 'Security Scan', icon: ShieldAlert, cmd: '/audit' }, { label: 'Unit Tests', icon: Code2, cmd: '/test' }, { label: 'Refactor', icon: Zap, cmd: '/refactor' }];
    ...
    <div className="flex-1 flex flex-col min-h-0 relative h-full bg-[#020202]/40 backdrop-blur-sm">
      <div className="flex-1 overflow-y-auto px-6 pb-40 custom-scrollbar scroll-smooth">
        <div className="max-w-4xl mx-auto space-y-10 pt-12">
          ...
          {messages.map((msg, idx) => <MessageBubble key={idx} msg={msg} theme={theme} />)}
          {isLoading && (
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#000000] via-[#000000] to-transparent pt-20 pb-4 pointer-events-none">
        <div className="max-w-3xl mx-auto pointer-events-auto flex flex-col gap-3 px-6">
           ...
           {/* REMOVED focus-within styles to eliminate white border */}
